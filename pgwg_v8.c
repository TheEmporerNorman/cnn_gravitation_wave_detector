...
typedef struct config_s {
	...
	uint32_t num_val_streams;
	double stream_duration; 

	//Wave configuration:
	uint32_t num_waves, num_val_waves;
	...
	double snr_cutoff;	
	double snr_min, snr_max, avg_snr, avg_val_snr;
	double noise_ajust;
	uint16_t num_gens;
	uint16_t num_epocs;
	...
	uint32_t num_glitches, num_val_glitches;
...
double calcSNR(config_s config, double* strain_axis, double* strain_axis_sig, double* strain_axis_noise){
	...
	calcPSD(config, strain_axis_noise_c, config.window_func, config.stream_res, 5, config.seg_length, &psd, &freq_axis); 
...
void sigSwitch(config_s config, uint16_t type, double amp, double tau, double f0, double alpha, double delta, double ciota, double mu, double* time_axis, double plus_polar, double* strain_axis, uint16_t detect_idx, double* centre_time_disps, double* orig_strain_axis)
{
	...
	switch(type){
		case 0: genChirpSig(config, amp*0.7, tau, f0, alpha, delta, ciota, mu, time_axis, plus_polar, strain_axis); break;
		case 1: genGaussSig(config, amp*0.7, tau, mu, time_axis, strain_axis); break;
		case 2: genRingSig(config, amp*6, tau, f0, delta, ciota, mu, time_axis, plus_polar, strain_axis); break;
		case 3: genSineSig(config, amp*6, f0,  mu,  time_axis, strain_axis); break;
		case 4: { 
			if (detect_idx == 0) { 
				genStatSig(config, 4*amp, tau, mu, time_axis, strain_axis); break; 
			}
			else {
				for (uint32_t time_idx = 0; time_idx < config.stream_res; time_idx++){
...
void setupConfig(config_s config, int argc, char** argv, config_s* ret_config)
{
	...
	config.num_val_streams = round((double) config.num_streams/4.0);
...
stream_s* initStreams(uint32_t num_streams)
{
	
	stream_s* streams = malloc(sizeof(stream_s)*num_streams);
...
stream_s* setupValStreams(config_s* config, stream_s* streams)
{

	double* num_glitches_present = malloc(sizeof(double)*config->num_val_streams*config->num_detects);
	genRandGaussArray(config->glitches_present_mu,config->glitches_present_sigma,config->num_val_streams*config->num_detects,num_glitches_present);

	config->num_val_glitches = 0;
	config->num_val_waves = 0;

	for (uint32_t stream_idx = 0; stream_idx < config->num_val_streams; stream_idx++){

		streams[stream_idx].num_waves_present = (uint16_t) round((double)rand_64()/(double)RAND_64_MAX);
		config->num_val_waves += (uint32_t) streams[stream_idx].num_waves_present;
		streams[stream_idx].num_glitches_present = malloc(sizeof(uint16_t)*config->num_detects);
		streams[stream_idx].snr = calloc(config->num_detects, sizeof(double)*config->num_detects);

		for (uint16_t detect_idx = 0; detect_idx < config->num_detects; detect_idx++){
			streams[stream_idx].num_glitches_present[detect_idx] = (uint16_t) abs(round(num_glitches_present[stream_idx*3 + detect_idx]));
			config->num_val_glitches += streams[stream_idx].num_glitches_present[detect_idx];
		}
...
void freeStreams_p(config_s config, stream_s* streams, uint32_t num_streams)
{
...
wave_s* setupWaves(config_s config, wave_s* waves, size_t num_waves, bool fixed_type, double type_min, double type_max, double wave_amp_mu)
{

	uint16_t* wave_types = malloc(sizeof(uint16_t)*num_waves); 
	genRandUIntArray(type_min, type_max, num_waves, wave_types);
	double* wave_amps = malloc(sizeof(double)*num_waves);
	genRandGaussArray(wave_amp_mu, config.wave_amp_sigma, num_waves, wave_amps); 
...
glitch_s* initGlitches(config_s config, uint32_t num_glitches)
{

	glitch_s* glitches = malloc(sizeof(glitch_s)*num_glitches);

	return glitches;
}


glitch_s* setupGlitches(config_s config, glitch_s* glitches, uint32_t num_glitches)
{
	
	uint16_t* glitch_types = malloc(sizeof(uint16_t)*num_glitches); 
	genRandUIntArray(0, config.num_glitch_types, num_glitches, glitch_types);
	double* glitch_amp = malloc(sizeof(double)*num_glitches);
	genRandGaussArray(config.glitch_amp_mu, config.glitch_amp_sigma, num_glitches, glitch_amp);
	double* glitch_tau = malloc(sizeof(double)*num_glitches);
	genRandGaussArray(config.glitch_tau_mu, config.glitch_tau_sigma, num_glitches, glitch_tau);
	double* glitch_f0s = malloc(sizeof(double)*num_glitches);
	genRandGaussArray(config.glitch_f0_mu, config.glitch_f0_sigma, num_glitches, glitch_f0s);
	double* glitch_alphas = malloc(sizeof(double)*num_glitches);
	genRandGaussArray(config.glitch_alpha_mu, config.glitch_alpha_sigma, num_glitches, glitch_alphas); 

	for (uint32_t glitch_idx = 0; glitch_idx < num_glitches; glitch_idx++){
...
uint32_t genWaves(config_s config, uint32_t stream_idx, uint32_t wave_count, stream_s* streams, wave_s* waves, detect_s* detects, stream_s** ret_streams, double wave_type_min, double wave_type_max, double wave_amp_mu)
{
	if (streams[stream_idx].num_waves_present > 0) {
		...
		if (calcbArrSum(amp_greater, config.num_detects) <= config.req_detects) { 
			...
			waves[wave_count] = setupWaves(config, &waves[wave_count], 1, 1, wave_type_min, wave_type_max, wave_amp_mu)[0]; 
			...
			wave_count = genWaves(config, stream_idx, wave_count, streams,  waves, detects, &streams, wave_type_min, wave_type_max, wave_amp_mu);
		...
		uint32_t max_idx;
		calcMax(streams[stream_idx].snr, config.num_detects, &streams[stream_idx].max_snr, &max_idx);
...
void genStreams(config_s config, stream_s* streams, wave_s* waves, detect_s* detects, glitch_s* glitches, stream_s** ret_streams, double wave_type_min, double wave_type_max, uint32_t num_streams, double wave_amp_mu)
{
...
void freeWaves(config_s config, wave_s* waves, uint32_t num_waves)
{
	for (size_t wave_idx = 0; wave_idx < num_waves; wave_idx++){
...
PyObject** makeArgArr(config_s config, stream_s* streams, stream_s* val_streams, size_t gen_idx, PyObject** args) 
{

    	args[0] = PyLong_FromUnsignedLong((unsigned long) config.num_streams);
    	args[1] = PyLong_FromUnsignedLong((unsigned long) config.num_val_streams);
		args[2] = PyLong_FromUnsignedLong((unsigned long) config.num_detects);
		args[3] = PyLong_FromUnsignedLong((unsigned long) config.stream_res);
		args[4] = PyLong_FromUnsignedLong((unsigned long) gen_idx);
		args[5] = PyLong_FromUnsignedLong((unsigned long) config.num_epocs);
		args[6] = PyList_New(config.num_streams);
		args[7] = PyList_New(config.num_streams*config.num_detects*config.stream_res);
		args[8] = PyList_New(config.num_streams*config.num_detects*config.stream_res);
		args[9] = PyList_New(config.num_val_streams);
		args[10] = PyList_New(config.num_val_streams*config.num_detects*config.stream_res);
		args[11] = PyList_New(config.num_val_streams*config.num_detects*config.stream_res);
		args[12] = makeListd(config.time_axis, config.stream_res);
		args[13] = PyBytes_FromString(config.file_path);


		for (uint32_t stream_idx = 0; stream_idx < config.num_streams; stream_idx++){
			PyList_SetItem(args[6] , stream_idx, PyLong_FromUnsignedLong((unsigned long) streams[stream_idx].num_waves_present));
			...
			for (uint32_t time_idx = 0; time_idx < config.num_detects*config.stream_res; time_idx++){
				...
				PyList_SetItem(args[7], idx, PyFloat_FromDouble(streams[stream_idx].strain_axes[time_idx]));
				PyList_SetItem(args[8], idx, PyFloat_FromDouble(streams[stream_idx].strain_axes_noise[time_idx]));
			} 
		}

		for (uint32_t stream_idx = 0; stream_idx < config.num_val_streams; stream_idx++){
			PyList_SetItem(args[9] , stream_idx, PyLong_FromUnsignedLong((unsigned long) val_streams[stream_idx].num_waves_present));
				
			for (uint32_t time_idx = 0; time_idx < config.num_detects*config.stream_res; time_idx++){
				size_t idx = stream_idx*config.num_detects*config.stream_res + time_idx; 
				PyList_SetItem(args[10], idx, PyFloat_FromDouble(val_streams[stream_idx].strain_axes[time_idx]));
				PyList_SetItem(args[11], idx, PyFloat_FromDouble(val_streams[stream_idx].strain_axes_noise[time_idx]));
			} 
		}
...
void plot(double* x_arr, uint32_t x_arr_len, double* y_arr, uint32_t y_arr_len, char* x_label, char* y_label, char* file_name)
{
	...
	runPythonFunc(num_args, "nnt_v6", "plotGraph", args, &output_arr, output_len);
}
...
void constructModel(config_s config, double* model_plan, size_t plan_length, uint16_t output_len, double** ret_output_arr)
{
	...
	runPythonFunc(num_args, "nnt_v6", "constructModel", args, &output_arr, output_len);
...
model_s initModel(config_s config)
{
	...
	model.avg_val_snr = malloc(sizeof(double)*config.num_gens);
...
void trainModel(config_s config, stream_s* streams, stream_s* val_streams, uint32_t gen_idx, model_s model){

		uint16_t output_len = 4;
		uint16_t num_args = 14; 
		PyObject** args = malloc(sizeof(PyObject*)* num_args);
		args = makeArgArr(config, streams, val_streams, gen_idx, args);

		freeStreams_p(config, streams, config.num_streams);
		freeStreams_p(config, val_streams, config.num_val_streams);

		double* output_arr = malloc(sizeof(double)*output_len);
		runPythonFunc(num_args, "nnt_v6", "main", args, &output_arr, output_len);
...
void printOutput(config_s config, model_s model){
	...
	for (size_t gen_idx = 0; gen_idx < config.num_gens; ++gen_idx){
		...
		fprintf(f, "%f,", model.avg_val_snr[gen_idx]);
...
void plotAccuracyGraph(config_s config, model_s model){
	...
	double* output_arr;
	runPythonFunc(5, "nnt_v6", "plotAccuracyGraph", args, &output_arr, 1);
...
int main(int argc, char** argv){
	...
	config.num_val_streams = round((double) config.num_streams/5.0);
	config.stream_sample_rate = 1000;
	config.stream_duration = 5.0;

	config.gen_waves = 1; 
	config.num_wave_types = 3;
	...
	config.amp_max = 1, config.amp_min = 0.5; 
	config.snr_cutoff_max = 0.01, config.snr_cutoff_min = 0.001; 
	config.amp_sigma_max = 0.001, config.amp_sigma_min = 0.0001;
	...
	stream_s* streams = initStreams(config.num_streams); 
	stream_s* val_streams = initStreams(config.num_val_streams); 
	...
	for (size_t gen_idx = 0; gen_idx < config.num_gens; gen_idx++){
		...
		val_streams = setupValStreams(&config, val_streams);
		...
		waves = setupWaves(config, waves, config.num_waves, 0, 0, config.num_wave_types, config.wave_amp_mu);

		wave_s* val_waves = initWaves(config, config.num_val_waves);
		val_waves = setupWaves(config, val_waves, config.num_val_waves, 0, val_min, val_max, config.wave_amp_mu*wave_amp_aj);
		...
		glitch_s* glitches = initGlitches(config, config.num_glitches);
		glitches = setupGlitches(config, glitches, config.num_glitches);

		glitch_s* val_glitches = initGlitches(config, config.num_val_glitches);
		val_glitches = setupGlitches(config, val_glitches, config.num_val_glitches);
		...
		genStreams(config, streams, waves, detects, glitches, &streams, 0, config.num_wave_types, config.num_streams, config.wave_amp_mu);
		genStreams(config, val_streams, val_waves, detects, val_glitches, &val_streams, val_min, val_max, config.num_val_streams,config.wave_amp_mu*wave_amp_aj);

		config.avg_snr = 0;
		config.avg_val_snr = 0;
		for (uint32_t stream_idx = 0; stream_idx < config.num_streams; stream_idx++){
			if (streams[stream_idx].num_waves_present == 1){
				config.avg_snr += streams[stream_idx].max_snr;
			}
			
		}

		for (uint32_t stream_idx = 0; stream_idx < config.num_val_streams; stream_idx++){
			if (val_streams[stream_idx].num_waves_present == 1){
				config.avg_val_snr += val_streams[stream_idx].max_snr;
			}	
		}

		config.avg_snr /= (double) (config.num_waves);
		config.avg_val_snr /= (double) (config.num_val_waves);

		model.avg_val_snr[gen_idx] = config.avg_val_snr;
		model.avg_snr[gen_idx] = config.avg_snr;

		freeWaves(config, waves, config.num_waves);
		freeWaves(config, val_waves, config.num_val_waves);
		free(glitches);
		free(val_glitches);
		...
		printf("Average val SNR: %f \n", config.avg_val_snr);

		trainModel(config, streams, val_streams, gen_idx, model);

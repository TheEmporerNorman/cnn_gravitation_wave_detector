...
def plotSampleWaves(num_detects,  strain_axis_noise, time_axis, strain_axis, waves_present, num_samples, num, file_path):
  ...
  plt.savefig(file_path + "/example_waves_"+ num + ".png")
...
def main(num_streams, num_detects, stream_res, gen_idx, num_epochs, num_waves_present, strain_axes, strain_axes_noise, time_axis, file_path): #num_waves_present
	
	file_path = file_path.decode("ascii")
	config = config_s() #<-- Creates config object

	strain_axes = np.asarray(strain_axes, dtype= np.float32)
	strain_axes = np.reshape(strain_axes, [num_streams, num_detects, stream_res], order = "C" )
	strain_axes_noise = np.asarray(strain_axes_noise, dtype= np.float32)
	strain_axes_noise = np.reshape(strain_axes_noise, [num_streams, num_detects, stream_res], order = "C" )
	num_waves_present = np.asarray(num_waves_present, dtype= np.int32)

	plotSampleWaves(config.num_detects, strain_axes_noise, time_axis, strain_axes, num_waves_present, 10, str(gen_idx), file_path)
